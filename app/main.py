...
async def health_check():
    ...
    try:
        ...
        fut = asyncio.get_event_loop().run_in_executor(
            None, lambda: socket.create_connection((redis_host, redis_port), timeout=2)
        )
...
async def root():
    ...
    return {
        ...
        "version": settings.VERSION,
        "environment": settings.ENVIRONMENT,
        "endpoints": {"docs": "/docs", "health": "/health", "api": settings.API_V1_STR},
    }
...
async def app_exception_handler(request: Request, exc: AppException):
    ...
    log_exception(exc, extra={"path": str(request.url.path), "method": request.method})
...
# Optional: run with 'python -m uvicorn app.main:app --reload'
